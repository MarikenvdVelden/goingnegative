...
data <- df  %>%
    mutate(Polls = branch(polls_calculation,
           ...
           new_party = branch(new_party_calculation,
            "np_option1" ~ new_party)) %>%
  branch_assert(!(ie_calculation == "mc_option3" & 
                    new_party_calculation == "np_option1")) %>%
  branch_assert(!(ie_calculation == "mc_option4" & 
                    new_party_calculation == "np_option1")) %>%
  branch_assert(!(ie_calculation == "mc_option7" & 
                    new_party_calculation == "np_option1")) %>%
  branch_assert(!(ie_calculation == "mc_option8" & 
                    new_party_calculation == "np_option1"))
})
...
inside(M, {
  fit_1 <- plm(direction_appeal ~ factor(medium) + Polls + IE + IO +
